...
void ASRPGPlayer::BeginPlay()
{
	Super::BeginPlay();

	if (CursorToWorld != nullptr)
	{
		pController = Cast<AGridTutPlayerController>(GetController());
		if(pController)
		{
...
// Called every frame
void ASRPGPlayer::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

}

void ASRPGPlayer::MoveUpDown(float rate_)
{
	if (bUnderControl)
	{
...
void ASRPGPlayer::MoveRightLeft(float rate_)
{
	if (bUnderControl)
	{
		FVector loc = GetActorLocation();
...
void ASRPGPlayer::SetUnderControl(bool value_)
{
	bUnderControl = value_;
}

void ASRPGPlayer::Zoom(float rate_)
{
	if (bUnderControl)
	{
